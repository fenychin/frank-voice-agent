...
### 2026-04-18: 本地STT + 云端LLM/TTS MVP架构

根据用户要求，Phase 1 MVP 使用本地STT + 云端LLM/TTS：

#### 变更内容

| 层级 | PDR原计划 | 变更后 | 原因 |
|------|-----------|--------|------|
| **STT** | Gemini 2.5 Flash STT (云) | **本地faster-whisper-large-v3-turbo** | 成本考虑 |
| **LLM** | Gemma 4 26B / Qwen3.5 | **MiniMax-M2.7-highspeed** | 用户指定 |
| **TTS** | Qwen3-TTS (本地) | **Qwen3-TTS-Flash** (DashScope) | 用户指定 |

#### 最终架构图

```
PC桌面端（本地Python应用）
  ├─ KWS唤醒 (sherpa-onnx) ← 本地
  ├─ VAD检测 (Silero) ← 本地
  ├─ 音频采集 ← 本地
  └─ STT (faster-whisper-large-v3-turbo) ← 本地
         ↓ 文字
┌─────────────────────────────────────────┐
│         Node.js 云端API网关              │
│  ┌─────────────────┐  ┌─────────────┐ │
│  │MiniMax-M2.7-    │→ │Qwen3-TTS-   │ │
│  │highspeed LLM    │  │Flash TTS    │ │
│  └─────────────────┘  └─────────────┘ │
└─────────────────────────────────────────┘
         ↓ MP3音频 / 文字
   本地播放 / 文字注入
...
- `src/server.js` - MiniMax LLM调用 + Qwen3-TTS-Flash TTS（移除云端STT）
- `app/api_client.py` - 本地faster-whisper STT → 云端LLM/TTS
- `app/tts_handler.py` - Qwen3-TTS-Flash TTS支持
- `app/config.py` - MINIMAX_API_KEY配置
- `.env.example` - 更新配置说明
...
# MiniMax (LLM)
MINIMAX_API_KEY=...
MINIMAX_LLM_MODEL=MiniMax-M2.7-highspeed

# DashScope (TTS)
DASHSCOPE_API_KEY=...
DASHSCOPE_TTS_MODEL=qwen-tts-flash
```
...
| 指标 | 目标 | 说明 |
|------|------|------|
| 端到端延迟 | < 2.5s | STT ~1s + LLM ~1s + TTS ~0.5s |
| STT准确率 | > 92% | 中英混合 |
| LLM响应速度 | < 500ms | MiniMax-M2.7-highspeed |
